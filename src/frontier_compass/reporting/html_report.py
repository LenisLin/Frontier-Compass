        ...
        *,
        ...
        summary_sections.extend(
            (
                ...
                _render_summary_panel("Frontier runtime", f"{digest.report_mode} / {digest.cost_mode}"),
                _render_summary_panel("Fetch scope", digest.fetch_scope or "n/a"),
                _render_summary_panel("Total fetched", str(max(digest.total_fetched, digest.total_ranked_count))),
                _render_summary_panel("Total displayed", str(digest.total_displayed_count)),
                _render_summary_panel("Timings", _timings_text(digest.run_timings) or "n/a"),
                _render_summary_panel("Recommended", str(brief.recommended_count)),
                _render_summary_panel(
                    ...
                    (
                        ...
                        else "Unavailable in legacy cache"
                    ),
                ),
                _render_summary_panel(
                    "Salient methods / topics",
                    (
        ...
        if source_mix_text:
            summary_sections.insert(
                ...
                _render_summary_panel("Source mix", source_mix_text),
            )
        if digest.search_profile_label:
            summary_sections.append(_render_summary_panel("Search profile", digest.search_profile_label))
        ...
        personalized_section_html = (
            '<p class="section-note">Recommendation-oriented reading pass through the current profile-aware shortlist.</p>'
        ...
        return _render_report_document(
            ...
            brief=brief,
            today_panel_html=today_panel_html,
            shortlist_title="Personalized Digest",
...
<head>
  ...
  <style>
    :root {{
      ...
      --bg: #120f0d;
      --bg-soft: #1e1814;
      --panel: rgba(27, 21, 18, 0.9);
      --panel-strong: rgba(19, 14, 11, 0.96);
      --ink: #f6efe4;
      --muted: #bba894;
      --accent: #d3a35f;
      --accent-soft: rgba(211, 163, 95, 0.14);
      --border: rgba(209, 180, 132, 0.2);
      --strong: #7ec2b3;
    ...
    * {{ box-sizing: border-box; }}
    body {{ margin: 0; font-family: var(--sans); background:
      ...
      radial-gradient(circle at top right, rgba(126, 194, 179, 0.14), transparent 20%),
    ...
    .meta {{ color: var(--muted); font-size: 0.95rem; letter-spacing: 0.03em; }}
    .summary {{ display: grid; gap: 12px; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); margin: 20px 0 14px; }}
    .panel {{ background: var(--panel); border: 1px solid var(--border); border-radius: 20px; padding: 16px; box-shadow: 0 18px 48px rgba(0, 0, 0, 0.24); backdrop-filter: blur(10px); }}
    .brief-grid {{ display: grid; gap: 14px; grid-template-columns: minmax(0, 1.3fr) minmax(0, 1fr); margin-bottom: 22px; }}
    .paper {{ background: var(--panel-strong); border: 1px solid var(--border); border-radius: 20px; padding: 20px; margin-bottom: 14px; box-shadow: 0 16px 36px rgba(0, 0, 0, 0.26); }}
    .paper.highlight {{ border-color: rgba(126, 194, 179, 0.34); box-shadow: 0 18px 42px rgba(126, 194, 179, 0.12); }}
    .paper h3 {{ margin: 0 0 8px; font-size: 1.25rem; }}
    .paper .score {{ color: var(--strong); font-weight: bold; }}
    .paper .status {{ display: inline-block; margin-bottom: 8px; border-radius: 999px; padding: 4px 10px; background: var(--accent-soft); color: #fff0d7; font-size: 0.84rem; font-weight: 700; }}
    .paper .theme {{ display: inline-block; margin: 0 0 8px 8px; border-radius: 999px; padding: 4px 10px; background: rgba(246, 239, 228, 0.08); color: var(--ink); font-size: 0.82rem; }}
    ...
    .chips li {{ border: 1px solid var(--border); border-radius: 999px; padding: 4px 10px; color: var(--muted); font-size: 0.9rem; }}
    ...
    .badge {{ display: inline-block; border-radius: 999px; padding: 4px 10px; background: rgba(246, 239, 228, 0.08); color: var(--ink); font-size: 0.82rem; }}
    a {{ color: var(--accent); text-decoration: none; }}
    a:hover {{ color: #f0c98f; }}
...
def daily_digest_title(digest: DailyDigest) -> str:
    ...
    if digest.category == BIOMEDICAL_MULTISOURCE_MODE:
        return f"FrontierCompass Biomedical Multisource Brief ({requested_window})"
...
def _daily_intro_text(digest: DailyDigest) -> str:
    ...
    if digest.category == BIOMEDICAL_LATEST_MODE:
        ...
        lead = "Reviewer-ready biomedical multisource brief using the fixed q-bio arXiv bundle plus same-day bioRxiv and medRxiv feeds."
    ...
    return (
        ...
        f"Profile: {digest.profile.basis_summary_label or digest.profile.basis_label or 'baseline only'}."
    )
...
def _display_contract_text(digest: DailyDigest) -> str:
    ...
    return (
        f"Personalized Digest surfaces {min(digest.total_ranked_count, 8)} shortlist items and "
...
def _render_frontier_report_section(frontier_report: DailyFrontierReport, *, profile: UserInterestProfile) -> str:
    scope_html = (
        f"<p><strong>Request window</strong><br>{escape(_request_window_text(frontier_report.request_window))}</p>"
        ...
        f"<p><strong>Source / mode</strong><br>{escape(frontier_report.source)} / {escape(frontier_report.mode_label or frontier_report.mode)}</p>"
        ...
        f"<p><strong>Fetch scope</strong><br>{escape(frontier_report.fetch_scope or 'n/a')}</p>"
        ...
        f"<p><strong>Source mix</strong><br>{escape(_source_count_summary(frontier_report.source_run_stats, frontier_report.source_counts) or 'n/a')}</p>"
    ...
    if frontier_report.profile_relevant_highlights:
        profile_highlights_html = (
            "<h3>Potentially relevant to your current profile</h3>"
    ...
    return (
        '<section class="section">'
        "<h2>Frontier Report</h2>"
        '<p class="section-note">Broader field scan built from the requested window’s fetched pool using title, abstract, and categories only. Profile relevance is shown separately and kept secondary.</p>'
        '<div class="brief-grid">'
        f'<div class="panel"><h2>What happened today in the field</h2><ul class="takeaways">{_render_frontier_takeaways(frontier_report.takeaways)}</ul></div>'
        f'<div class="panel"><h2>Field scope</h2>{scope_html}</div>'
        "</div>"
        f"{status_panel}"
        '<div class="summary">'
        f'{_render_summary_panel("Repeated themes", _signal_text(frontier_report.repeated_themes) or "No repeated theme signal")}'
        f'{_render_summary_panel("Salient methods / topics", _signal_text(frontier_report.salient_topics) or "No dominant topic bucket")}'
        f'{_render_summary_panel("Adjacent themes", _signal_text(frontier_report.adjacent_themes) or "No notable adjacent theme")}'
        "</div>"
        "<h3>Field-wide highlights</h3>"
...
def _render_missing_frontier_report_section() -> str:
    return (
        ...
        '<div class="panel"><p><strong>Frontier report unavailable</strong><br>'
...
def _render_frontier_highlight_card(
    ...
    *,
    ...
    if show_score and item.score is not None:
        score_html = f'<div class="score">Profile score {item.score:.3f}</div>'
    return f"""<article class="paper highlight">
  <div class="status">{escape(_format_source_label(item.source))}</div>
  <div class="theme">Theme: {escape(item.theme_label)}</div>
  {score_html}
  <h3>{title_html}</h3>
  <div class="meta">{meta}</div>
  <p><strong>Why it matters</strong><br>{escape(item.why)}</p>
...
def _render_card(
    ...
    *,
    ...
    reason_label: str = "Why it surfaced",
    ...
    why_it_surfaced = reason_text if reason_text is not None else explanation_summary_line(explanation)
    ...
    meta_parts = [escape(_format_source_label(paper.source)), escape(authors)]
    if paper.published:
        meta_parts.insert(1, escape(f"published {paper.published.isoformat()}"))
    if paper.updated and paper.updated != paper.published:
        meta_parts.insert(2, escape(f"updated {paper.updated.isoformat()}"))
    ...
    theme_html = f'<div class="theme">Theme: {escape(theme_label_for_ranked_paper(item))}</div>' if highlight else ""
